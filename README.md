...
| hasRowsChanged       | `function`         | `rows.length,row.id,row.modifiedAt` | Signature - `(rows) => Key: String`. Function is called to detect whether rows props has changed. Default function detects changes on length and id, modifiedAt fields       |
